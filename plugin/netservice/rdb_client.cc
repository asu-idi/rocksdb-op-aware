...
NetClient::NetClient(std::shared_ptr<grpc::Channel> channel, int batch_size,
                     size_t queue_size)
...
void NetClient::SetOperation(OperationRequest& request,
...
bool NetClient::SingleWriter(const std::string& operation,
...
bool NetClient::BufferedWriter(const std::string& operation,
...
//     while (running_ || !request_queue_.empty()) {
//         std::unique_lock<std::mutex> lock(mutex_);
//         condition_.wait(lock, [this] { return !request_queue_.empty() ||
//         !running_; });
...
//         SendRequest(request);
//         condition_.notify_one(); // Notify BufferedWriter that space is
//         available
//     }
// }
